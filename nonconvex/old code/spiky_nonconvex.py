...
class SpikyNonconvexCoordinateDescent:
    ...
    where a_i ∈ (0,1], x_i ∈ [-1,1], and w_i are frequency parameters for each query.
    
    NOTE: This version is designed for NONCONVEX spiky functions with per-query frequency parameters.
    """
    ...
    def __init__(self, 
        ...
        Args:
            ...
            frequency: Frequency parameter w for sin(w*x) term
        ...
        self.amplitudes_matrix = None  # Matrix of a_i vectors for each query: shape (k, n)
        self.frequencies_vector = None  # Vector of w_i frequency parameters for each query: shape (k,)
        
    def set_queries_and_amplitudes(self, amplitudes_matrix: np.ndarray, frequencies_vector: np.ndarray, weights: List[float]):
        """
        Set the amplitude matrix, frequency vector, and weights for spiky nonconvex queries.
        
        Args:
            amplitudes_matrix: Matrix of amplitude vectors for each query, shape (k, n)
                              Each row represents a_i amplitudes for one query
            frequencies_vector: Vector of frequency parameters for each query, shape (k,)
                               Each element represents w_i frequency for one query
        ...
        self.sampled_queries = np.array([2.0] * k)  # All spiky queries
    ...
    def compute_query_outputs(self):
        ...
        for index in range(k):
            # Get amplitude vector and frequency for this query
            amps = self.amplitudes_matrix[index]
            freq = self.frequencies_vector[index]
            
            # Real data output: (1/n) * sum_i (x_i^2 + a_i * sin(w_i*x_i))
            ...
            # Fake data output: (1/n) * sum_i (x_i^2 + a_i * sin(w_i*x_i))
    ...
    def coordinate_descent_step(self) -> Tuple[float, float]:
        ...
        for i in range(self.num_points):
            ...
            gradient_sum = 0.0
            for j in range(k):
                # Get amplitude and frequency for this query and coordinate
                a_i = self.amplitudes_matrix[j, i]
                w_i = self.frequencies_vector[j]
                
                # dq/dx_i = (1/n) * (2*x_i + w_i*a_i * cos(w_i*x_i))
                query_grad_wrt_xi = (1.0 / self.n) * (2 * xi + w_i * a_i * np.cos(w_i * xi))
        ...
        # # Debug: Check if gradients are too small
        # max_grad = np.max(np.abs(loss_gradient))
        # if max_grad < 1e-10:
        #     print(f"Warning: Maximum gradient is very small: {max_grad}")
        ...
        # for j in range(k):
        #     # Get amplitude and frequency for this query and coordinate
        #     a_i = self.amplitudes_matrix[j, x_coord_descent]
        #     w_i = self.frequencies_vector[j]
            
        #     # dq/dx_i = (1/n) * (2*x_i + w_i*a_i * cos(w_i*x_i)), d2q/dx_i2 = (1/n) * (2 - w_i^2*a_i * sin(w_i*x_i))
        #     query_grad_wrt_xi = (1.0 / self.n) * (2 * xi + w_i * a_i * np.cos(w_i * xi))
        #     query_hessian_wrt_xi = (1.0 / self.n) * (2 - w_i**2 * a_i * np.sin(w_i * xi))
        ...
        # # Debug: Check Hessian
        # if abs(loss_hessian_wrt_chosen_x_coord) < 1e-10:
        #     print(f"Warning: Hessian is very small: {loss_hessian_wrt_chosen_x_coord}")
        #     return total_loss, 0.0
        
        # # Debug: Print gradient and Hessian info (commented out for cleaner output)
        # # print(f"Debug: Max gradient = {max_grad:.6e}, Hessian = {loss_hessian_wrt_chosen_x_coord:.6e}")
        ...
        #         amps = self.amplitudes_matrix[index]
        #         freq = self.frequencies_vector[index]
        #         sum_fake_linesearch = np.sum(fake_X_copy_linesearch**2 + amps * np.sin(freq * fake_X_copy_linesearch))
        ...
        # === Start of multi-coordinate gradient-descent replacement (Fix B) ===

        # Update up to m coordinates with largest |gradient|
        m = min(20, self.num_points)     # you can tune 5–20
        idxs = np.argsort(-np.abs(loss_gradient))[:m]

        # If gradient is tiny everywhere, bail early
        if np.max(np.abs(loss_gradient[idxs])) < 1e-10:
            return total_loss, 0.0

        # Descent direction on those m coordinates (zeros elsewhere)
        direction_vec = np.zeros_like(self.fake_X)
        direction_vec[idxs] = -loss_gradient[idxs]

        # Backtracking (Armijo) on a single scalar step for the block
        t = 1.0
        alpha = 1e-3      # more permissive than 0.05
        beta  = 0.7       # shrink slower than 0.5
        t_min = 1e-6      # fallback step if backtracking fails

        k = self.k  # shorthand

        def eval_loss_with_step(step):
            # Propose x_new and project to bounds
            x_new = self.fake_X + step * direction_vec
            x_new = np.clip(x_new, self.lower_bound, self.upper_bound)

            # Recompute fake outputs for all queries
            fake_output_new = np.zeros(k)
            for idx in range(k):
                amps = self.amplitudes_matrix[idx]
                freq = self.frequencies_vector[idx]
                s = np.sum(x_new**2 + amps * np.sin(freq * x_new))
                fake_output_new[idx] = s / self.n
            ...
            return loss_plus + loss_minus, fake_output_new, x_new

        # Current loss already computed above as total_loss
        # Directional derivative g^T d (only over updated coords)
        dir_deriv = np.dot(loss_gradient[idxs], direction_vec[idxs])  # = -||grad_top||^2 <= 0

        # Armijo backtracking
        loss_try, fake_output_try, x_try = None, None, None
        while True:
            loss_try, fake_output_try, x_try = eval_loss_with_step(t)
            if loss_try <= total_loss + alpha * t * dir_deriv:
                break
            t *= beta
            if t < t_min:
                # Fallback: take a tiny step anyway to avoid stalling
                t = t_min
                loss_try, fake_output_try, x_try = eval_loss_with_step(t)
                break
        ...
        # Update error and recompute total loss
        ...
        total_loss_update = total_loss - total_loss_after
        return total_loss_after, total_loss_update
        # === End of multi-coordinate gradient-descent replacement (Fix B) ===
    ...
    def run_coordinate_descent(self, max_iterations: int = 1000, verbose: bool = True) -> dict:
        ...
        total_loss = float('inf')  # Initialize total_loss
...
def run_lp_norms_optimization(queries: List[float], 
    ...
    # Set amplitude matrix, frequency vector, and weights (for compatibility, create dummy matrices)
    # This is a legacy function - use set_queries_and_amplitudes for proper functionality
    amplitudes_matrix = np.ones((len(queries), n))  # All ones for compatibility
    frequencies_vector = np.ones(len(queries)) * 5.0  # Default frequency 5.0 for compatibility
...
if __name__ == "__main__":
    ...
    print("Use test_spiky_nonconvex.py for testing with different parameters.")

...
def test_spiky_nonconvex_queries(
    n=200, epsilon=3.0, delta=1e-5, beta=0.1, eta=0.01,
    k=20, max_iterations=1000, seed=None
):
    # Pick a seed if none
    if seed is None:
        seed = secrets.randbits(32)
    ...
    frequencies_vector = np.random.uniform(1.0, 10.0, size=k)
    weights = [1.0 / k] * k
...
def test_parameter_sweep():
    for n in [50, 100, 200]:
        test_spiky_nonconvex_queries(n=n, k=10, epsilon=3.0, delta=1e-5, beta=0.1, eta=0.01, max_iterations=1000)

    for epsilon in [0.5, 1.0, 3.0, 5.0]:
        test_spiky_nonconvex_queries(n=200, k=10, epsilon=epsilon, delta=1e-5, beta=0.1, eta=0.01, max_iterations=1000)

    for k in [5, 10, 20, 50]:
        test_spiky_nonconvex_queries(n=200, k=k, epsilon=3.0, delta=1e-5, beta=0.1, eta=0.01, max_iterations=1000)
...
if __name__ == "__main__":
    ...
    test_multiple_runs(num_runs=3, n=200, epsilon=3.0, k=20, delta=1e-5, beta=0.1, eta=0.01)
